...
public class BelchBomb extends AbilityProvider {
    ...
    private final List<String> belchbombmessagedamager = CC.translate(PurgeAbilities.getInstance().getConfig().getStringList("BelchBomb.Messages.Damager"));
    private final List<String> belchbombmessagedamaged = CC.translate(PurgeAbilities.getInstance().getConfig().getStringList("BelchBomb.Messages.Damaged"));
    ...
    public String getDisplayName() {
        return CC.translate(config.getString("BelchBomb.Item.Name"));
    }

    @Override
    public List<String> getLore() {
        return CC.translate(config.getStringList("BelchBomb.Item.Lore"));
    }

    @Override
    public int getCooldown() {
        return config.getInt("BelchBomb.Cooldown");
    }

    @Override
    public ItemStack getItemStack() {
        return new ItemStack(Material.valueOf(config.getString("BelchBomb.Item.Material")));
    }

    @Override
    public boolean isEnabled() {
        return config.getBoolean("BelchBomb.Enabled");
    }

    @Override
    public String getInfo() {
        return CC.translate("The BelchBomb ability item is a ability item where you can right-click the item to give players in a set radius debuff effects.");
    }
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        Cooldown cooldown = PurgeAbilities.getInstance().getCooldowns().getBelchbomb();
        ...
        for (String s : belchbombmessagedamager) {
            player.sendMessage(CC.translate(s).replace("%heart%", "❤").replace("%ability%", getDisplayName()));
        }
        for (Entity e : player.getNearbyEntities(config.getInt("BelchBomb.Range"), config.getInt("BelchBomb.Range"), config.getInt("BelchBomb.Range"))) {
            if (!(e instanceof Player)) return;
            Player entity = (Player) e;
            for (String s : belchbombmessagedamaged) {
                entity.sendMessage(s.replace("%time%", cooldown.getRemaining(player)).replace("%heart%", "❤").replace("%ability%", getDisplayName()));
            ...
            for (String key : PurgeAbilities.getInstance().getConfig().getConfigurationSection("BelchBomb.Effects").getKeys(false)) {
                PotionEffect ps = new PotionEffect(PotionEffectType.getByName(config.getString("BelchBomb.Effects." + key + ".Effect")), config.getInt("BelchBomb.Effects." + key + ".Time") * 20, config.getInt("BelchBomb.Effects." + key + ".Level") - 1);
                entity.addPotionEffect(ps);
            }
